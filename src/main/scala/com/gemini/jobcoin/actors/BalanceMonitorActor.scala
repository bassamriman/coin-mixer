...
case class BalanceMonitorActor() extends MixerActor {
  ...
  def handle(requestAwaitingBalance: Map[String, MixRequest],
    ...
    case NewRequestsAwaitingBalance(mixRequests) =>
      ...
      val newMixRequestIdToSender = mixRequestIdToSender ++ mixRequests.map(mixRequest => mixRequest.id -> sender)
      context.become(
        handle(
    ...
    case LedgerActor.LatestLedger(newLedger) =>
      ...
      val mixRequestsBalancePairs: Seq[(MixRequest, BigDecimal)] =
        mixRequestIdToBalance.toSeq.flatMap(entry => requestAwaitingBalance.get(entry._1).map(_ -> entry._2))
      ...
      val groupedMixRequestsBalancePairs: Map[ActorRef, Seq[(MixRequest, BigDecimal)]] =
        mixRequestsBalancePairs.groupBy(mixRequestBalancePair => mixRequestIdToSender(mixRequestBalancePair._1.id))

      val now = LocalDateTime.now()
      groupedMixRequestsBalancePairs.foreach {
        senderToMixRequestsBalancePairs =>
          val (sender, mixRequestsBalancePairs) = senderToMixRequestsBalancePairs
...
object BalanceMonitorActor {
  ...
  case class BalanceReceived(balanceMixRequestPairs: Seq[(BigDecimal, MixRequest)], timestamp: LocalDateTime)

  case class BalanceNotReceived(mixRequests: Seq[MixRequest], timestamp: LocalDateTime)
